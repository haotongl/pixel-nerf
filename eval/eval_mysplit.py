...
with torch.no_grad():
    for obj_idx, data in enumerate(data_loader):
        ...
        images_new = []
        for img_idx in range(NV):
            image_path = os.path.join('data/dtu/Rectified', f'{obj_name}_train','rect_{:03d}_3_r5000.png'.format(img_idx+1))
            img = ((np.array(imageio.imread(image_path)) / 255.) * 2. - 1.).astype(np.float32)
            images_new.append(torch.tensor(img))
        images_new = torch.stack(images_new)
        images = images_new.permute(0, 3, 1, 2)
        NV, _, H, W = images.shape
        data['focal'] = data['focal'] * 2.
        data['c'][0][0] = (data['c'][0][0] - 40) * 2.
        data['c'][0][1] = (data['c'][0][1] - 22) * 2.


        near_dict = {23: [15, 22, 33], 24: [14, 25, 33], 32: [31, 33, 25], 44: [43, 33, 31]}
        for k in near_dict:
            source = torch.Tensor(near_dict[k]).long()
            all_rays = None
