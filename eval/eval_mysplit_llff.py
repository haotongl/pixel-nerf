...
with torch.no_grad():
    for obj_idx, data in enumerate(data_loader):
        ...
        llff_split = np.load('llff_split.npy', allow_pickle=True).item()

        for key in llff_split:
            if obj_name not in key:
                continue
            scene, view_idx =  key.split('_')
            tar_view_idx = int(view_idx)
            source = torch.tensor(llff_split[key]).long()
            all_rays = None
            ...
            if all_rays is None or use_source_lut or args.free_pose:
                ...
                target = torch.Tensor([tar_view_idx]).long()
                target_view_mask = torch.zeros(NV, dtype=torch.bool)
